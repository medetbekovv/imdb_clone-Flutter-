...
class MovieRepositoryImpl implements MovieRepository {
  ...
  @override
  Future<List<MovieData>> getTrendingMoviesOfThisWeek() async {
    print('Fetching trending movies from API'); // Логируем начало запроса
    final paginatedResponse = await _movieApi.getTrendingMoviesOfThisWeek();
    print('Received response: ${paginatedResponse.results}'); // Логируем данные
    final movies = paginatedResponse.results;
    final data = movies.map((movie) {
      print('Mapping movie: $movie'); // Логируем процесс маппинга
      return movie.toData();
    }).toList();
    print('Mapped movies: $data'); // Логируем результат маппинга
    return data;
  }
